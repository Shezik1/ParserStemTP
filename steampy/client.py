...
class SteamClient:
    ...
    def her(self):
        ...
        item_sold = []
        while i3 > 0:
            hui = self._session.get(f"https://steamcommunity.com/market/myhistory/render/?query=&start={i3}&count=100").text.split('"')
            i2 = 0
            i = 0
            while i2 != 100:
                while 0 != 10:
                    try:
                        if "market_listing_left_cell market_listing_gainorloss" in hui[i]:
                            if "-" in hui[i + 1]:
                                tradable = "Продажа"
                                break
                            if "+" in hui[i + 1]:
                ...
                if tradable == "Размещён":
                    ...
                    while 0 != 10:
                        ...
                        i += 1
                    while 0 != 10:
                        if "market_listing_item_name" in hui[i] and "market_listing_item_name_block" not in hui[i]:
                            item_name = hui[i + 3].split("<")[0].split(">")[1]
                            break
                        i += 1
                    while 0 != 10:
                        if "market_listing_listed_date_combined" in hui[i]:
                            date = hui[i + 1].split("\\")[5].split(":")[1].lstrip()
                            break
                        i += 1
                    if tradable == "Покупка":
                        print(Fore.GREEN + str(i2), tradable, "(" + item_name + ")", date, price)
                        item_buy.append(f"{tradable} @{item_name}@ {date} {price}")
                    if tradable == "Продажа":
                        print(Fore.RED + str(i2), tradable, "(" + item_name + ")", date, price)
                        item_sold.append(f"{tradable} @{item_name}@ {date} {price}")
                    i2 += 1
            i3 -= 100
            time.sleep(1)
        return item_buy, item_sold
